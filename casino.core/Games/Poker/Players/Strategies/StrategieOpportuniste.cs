...
public class StrategieOpportuniste : IStrategiePlayer
{
    public ActionJeu ProposerAction(ContexteDeJeu contexte)
    {
        ...
        int nbActifs = Math.Max(2, contexte.Partie.Players.Count(j => !j.IsFolded()));
        Phase phase = contexte.Partie.Phase;

        // pBrut est en 0..100
        double pBrut = EvaluateurProbabilite.EstimerProbabiliteDeGagner(
            contexte.PlayerCourant.Hand,
            contexte.Partie.CommunityCards,
            contexte.Partie.Players.Count,
            simulations: 1000);
        ...
        if (p < seuilRelance)
        {
            ...
            if (actions.Contains(TypeActionJeu.Miser))
                return new ActionJeu(TypeActionJeu.Miser, contexte.MinimumBet);
        ...
        if (actions.Contains(TypeActionJeu.Relancer))
            return new ActionJeu(TypeActionJeu.Relancer, contexte.MinimumBet);
